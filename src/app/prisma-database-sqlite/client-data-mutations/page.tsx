...
export default function DataMutations() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        setLoading(true)
        try {
            const response = await fetch('/prisma-database-sqlite/client-data-mutations/api', {
                method: 'POST',
            ...
            if (response.ok) {
                router.push('/prisma-database-sqlite')
            }
        } catch (err) {
            setError("Data Handle Submit Errpr")
        } finally {
            setLoading(false)
        }
    };
    return (
        <div className="m-10 min-h-7/12">
            <div className="flex gap-10">
                <h1>This is Data Mutations Page T-70</h1>
                <div>
